...
-- 2) A los que ya existen se les devuelve su precio.
update public.products p
  ...
  ) as v(id, name, price, category, active)
 where p.id = v.id;

-- 3) Si alguno faltara, se agrega con el mismo dueño que los demás.
--    (El editor de Supabase no entra firmado como el negocio, por eso se copia.)
insert into public.products (id, owner, name, price, category, active)
select v.id,
       coalesce(
         (select owner from public.products where owner is not null limit 1),
         (select id    from auth.users     order by created_at limit 1)
       ),
       v.name, v.price, v.category, v.active
  from (values
    ('17c3dfce-dfac-4ab7-a4f2-79bdba169911', 'Té', 35, 'BEBIDAS', true),
    ('p8', 'Torta de birria', 120, 'ALIMENTOS', true),
    ('p9', 'Refresco', 35, 'BEBIDAS', true),
...
-- 4) El menú de fábrica revivió un producto que tú ya habías borrado
--    (p4). Se quita de nuevo.
delete from public.products where id in ('p4');

-- 5) Comprobación: así queda el menú.
